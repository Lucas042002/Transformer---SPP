...
def recursive_packing(space, spaces, rects, placed, estados=None, Y_rect=None):
    while rects:
        i = random.randrange(len(rects))
        rect = rects[i]

        # Prueba ambas orientaciones (sin rotar y rotado)
...
def hr_packing_random(spaces, rects):
    placed = []
    rects1 = rects.copy()
...
def heuristic_recursion(rects, container_width):
    ...
    for i in range(len(rects) - 1):
        for j in range(i + 1, len(rects)):
            ...
            placements, estados, Y_rect = hr_packing_random(
            ...
            visualizar_packing(placements, container_width, container_height=20, show=True)
